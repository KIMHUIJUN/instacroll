...
for i in range(20):  # 최근사진 갯수 = ( x + 3 ) x 3 +
    a = 1

    for p in range(3):
        thumbnails = driver.find_element(By.XPATH,'//*[@id="react-root"]/section/main/article/div[2]/div/div[{}]/div[{}]/a/div/div[1]/img'.format(n2, a)).get_attribute('src')
        time.sleep(3)
        urllib.request.urlretrieve(thumbnails, "{}.jpg".format(s))
        time.sleep(3)
        print(n2 , a)
